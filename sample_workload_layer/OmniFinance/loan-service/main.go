...
func exportHandler(w http.ResponseWriter, r *http.Request) {
    ...
    // 2. RCE EXECUTION (Conditional Vulnerability for Demo)
    var out []byte

    // We check if the input contains common shell chaining characters
    if strings.ContainsAny(loanID, ";&|$`") {
        // 🔴 LEGACY/VULNERABLE PATH: Spawns a shell. AuraNet WILL catch this.
        fmt.Printf("[Loan Service] Legacy shell exporter invoked...\n")
        cmdStr := fmt.Sprintf("echo Exporting data for Loan ID: %s", loanID)
        cmd := exec.Command("sh", "-c", cmdStr)
        out, _ = cmd.CombinedOutput()
    } else {
        // 🟢 SAFE PATH: Uses native Go strings. No shell spawned. AuraNet ignores this.
        fmt.Printf("[Loan Service] Native Go exporter invoked...\n")
        safeOutput := fmt.Sprintf("Exporting data for Loan ID: %s\n", loanID)
        out = []byte(safeOutput)
    }
