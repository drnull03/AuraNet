  ...
  return (
    <div className="bg-white border border-brand-border rounded-2xl shadow-sm p-5 flex flex-col h-[640px]" id="network-topology-card">
      ...
      <div className="flex-1 min-h-[400px] border border-slate-100 rounded-xl relative overflow-hidden bg-slate-50/50">
        ...
        {/* REACTIVE EVENT TICKER (PERSISTING MESSAGES) */}
        <div className="absolute top-0 left-0 right-0 z-20 bg-slate-900/90 font-mono text-[10.5px] py-2 px-4 flex items-start gap-3 backdrop-blur-md shadow-lg border-b border-emerald-500/30">
          <div className="flex items-center gap-1.5 text-emerald-300 font-bold tracking-widest uppercase flex-shrink-0 mt-0.5">
            <Activity size={14} className="animate-pulse" /> Live Feed
          </div>
          <div className="w-px h-4 bg-emerald-500/40 mx-1 mt-0.5" />
          <div className="flex-1 flex flex-col gap-1 max-h-[50px] overflow-y-auto custom-scrollbar">
            {liveEvents.length > 0 ? (
              liveEvents.map((ev) => (
                <span key={ev.id} className="animate-fade-in-right text-[#00ced1]">{ev.text}</span>
              ))
            ) : (
              <span className="text-[#00ced1]">🟢 SECURE: Awaiting telemetry signals from AuraNet Engine & Runtime...</span>
            )}
          </div>
        </div>
      ...
      {/* Selected Node Details Tray (With Logs below) */}
      {selectedNode && (
        <div className="mt-4 bg-[#f8f9fa] border border-brand-border rounded-xl flex flex-col relative transition-all duration-300 overflow-hidden">
