...
// Custom Central Node component (kept for fallback compatibility)
...
const SatelliteNodeComponent = ({ data }: any) => {
  ...
  const getStatusColor = () => {
    ...
    return 'bg-cyan-700 border-cyan-500 shadow-cyan-700/30'; // Darker cyan
  };
...
export default function NetworkFlow({
  ...
  // Update flow elements when our parent state changes (SMART MERGE FIX)
  useEffect(() => {
    const freshNodes = getInitialNodes();
    
    setNodes((currentNodes) => {
      return freshNodes.map(freshNode => {
        // Find if this node is already rendered on the canvas
        const existingNode = currentNodes.find(n => n.id === freshNode.id);
        if (existingNode) {
          // Keep the existing node's X/Y position to prevent dragging resets!
          return {
            ...freshNode,
            position: existingNode.position
          };
        }
        // If it's a brand new node, use the auto-calculated grid position
        return freshNode;
      });
    });

    // Edges can be fully replaced since they don't have drag state
  ...
  useEffect(() => {
    ...
    fetchTopology();
    // Poll every 5 seconds to keep the dashboard live with the cluster
  ...
  return (
    <div className="bg-white border border-brand-border rounded-2xl shadow-sm p-5 flex flex-col h-[640px]" id="network-topology-card">
      ...
      {selectedNode && (
        <div
          className="mt-4 bg-[#f8f9fa] border border-brand-border rounded-xl p-3 flex flex-wrap items-center justify-between gap-3 relative transition-all duration-300"
            ...
            <div className="flex items-center gap-1.5 border-l border-slate-200 pl-3">
              ...
              <button
                onClick={() => {
                  ...
                  onNodeSelect(systemNodes[0] || null); // Reset to another node safely
                }}
