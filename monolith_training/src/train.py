...
def train_pipeline():
    print("🐝 [MLOps] Initializing Training Pipeline...")
    ...
    # 1. Process Data
    print("📊 [MLOps] Processing raw training telemetry...")
    ...
    print(f"✅ [MLOps] Loaded {len(torch_dataset)} pure network events ({input_dim} dimensions).")

    # Save the tensor for future use
    os.makedirs(os.path.dirname(config.PROCESSED_TENSOR_PATH), exist_ok=True)
    torch.save(torch_dataset.data, config.PROCESSED_TENSOR_PATH)
    ...
    # 5. Save the Brain to the new path specified in your config.py
