...
def evaluate_pipeline():
    ...
    if not os.path.exists(config.TEST_DATA_PATH):
        print(f"❌ Error: Test data not found at {config.TEST_DATA_PATH}. Run the test_simulator.py!")
        return

    # 1. Process Test Traffic
    ...
    for i in range(len(df)):
        ...
        if mse_loss > config.TRIPWIRE_THRESHOLD:
            ...
            print(f"   Action:  Triggering AutoHeal for {context['src_app']}...\n")
