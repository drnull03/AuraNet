"""
@file nlp_model.py
@brief URL and HTTP payload NLP anomaly detection models.

Defines an LSTM-based autoencoder architecture used to detect
malicious URL structures and HTTP payload anomalies.
"""
import torch
import torch.nn as nn
"""
@brief LSTM autoencoder for sequence anomaly detection.

Uses character-level embeddings and LSTM encoder-decoder layers
to learn normal URL or HTTP body structures.
"""
...
Encodes the input sequence, reconstructs it through the decoder,
and returns raw logits for CrossEntropy-based anomaly scoring.

@param x Input token sequence.
@return Reconstructed sequence logits.
"""
    def forward(self, x):
