/**
 * @file index.js
 * @brief AuraNet Runtime Forwarder microservice.
 *
 * Collects runtime security events generated by eBPF/Tetragon,
 * analyzes process execution and kernel file access events,
 * applies threat classification rules, and forwards detected
 * security alerts to the AuraNet Zero Trust Controller through NATS.
 *
 * Runs as a node-level component responsible for runtime visibility
 * and threat event forwarding.
 */
...
/**
 * Starts the AuraNet Runtime Forwarder.
 *
 * Establishes the NATS connection, monitors the runtime security log
 * stream, parses eBPF/Tetragon events, detects suspicious behavior,
 * performs alert deduplication, and publishes validated threats to
 * the AuraNet event pipeline.
 *
 * @async
...
async function startForwarder() {
    ...
    try {
        ...
        // Graceful shutdown
        /**
 * Registers graceful shutdown handlers for the runtime forwarder.
 *
 * Stops log monitoring and closes the NATS connection when the
 * Kubernetes workload receives termination signals.
 *
 * @param {string} signal Operating system termination signal.
 * @returns {void}
 */
